...
import { 
  ArrowLeft, Wand2, Plus, Eye, Share, 
  CheckCircle2, GripVertical, Trash2, X, Copy 
} from 'lucide-react';
import { 
...
export default function Builder() {
  ...
  const saveForm = useCallback(async (updatedForm: Form) => {
    if (!id) return;
    setSaving(true);
    try {
  ...
  const publishForm = async () => {
    if (!form) return;
    const updatedForm = { ...form, status: 'published' as const, publishedAt: Date.now() };
    await saveForm(updatedForm);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          {saving && <span className="text-xs text-slate-500 flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> Saved</span>}
          <button onClick={() => navigate(`/f/${form.id}`)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Preview">
          ...
          <button 
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors ${
              form.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-blue-600 text-white hover:bg-blue-700'
            }`}
          >
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full p-4 md:p-8">
        ...
        {form.questions.length === 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8 relative overflow-hidden">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2 relative z-10">Describe the form you want to create</h2>
            <p className="text-slate-500 text-sm mb-6 relative z-10">Let AI build it for you instantly.</p>
            ...
            <div className="relative z-10">
              ...
              >
                {generating ? 'Generating...' : <><Wand2 className="w-4 h-4" /> Generate Form</>}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6">
              <div className="flex flex-col items-center mb-6">
                ...
                <p className="text-sm text-slate-500">Scan QR to fill out form</p>
              </div>
              
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Form Link</label>
...
const SortableQuestion: React.FC<SortableQuestionProps> = ({ question, update, remove }) => {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: question.id });
  const style = { transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 10 : 1 };

  return (
    <div ref={setNodeRef} style={style} className={`bg-white rounded-xl shadow-sm border ${isDragging ? 'border-blue-500 shadow-md' : 'border-slate-200'}`}>
      <div className="flex">
        {/* Drag Handle */}
        <div {...attributes} {...listeners} className="w-8 flex flex-col items-center justify-center border-r border-slate-100 cursor-grab active:cursor-grabbing text-slate-300 hover:text-slate-500">
        ...
        <div className="p-6 flex-1">
            ...
            <div className="space-y-2 ml-2">
              {(question.options || []).map((opt, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className={`w-4 h-4 border border-slate-300 ${question.type === 'multiple_choice' ? 'rounded-full' : 'rounded-sm'}`} />
                  <input 
                    type="text" 
                    value={opt}
                    onChange={e => {
          ...
          <div className="flex items-center justify-end gap-4 mt-6 pt-4 border-t border-slate-100">
            <button onClick={remove} className="text-slate-400 hover:text-red-500 p-2 rounded-full hover:bg-slate-50">
